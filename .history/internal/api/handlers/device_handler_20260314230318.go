...
func (handler *DeviceHandler) GetDeviceByID(context *gin.Context) {
	...
	if state.Type == "light-sensor" {
		if luxVal, exists := state.Payload["light_level"]; exists {
			var lux float64
			// Safely assert the type (DynamoDB might return it as float64 or int)
			if floatVal, ok := luxVal.(float64); ok {
				lux = floatVal
			} else if intVal, ok := luxVal.(int); ok {
				lux = float64(intVal)
			}
			
			// Calculate the status based on standard lux thresholds
			status := "Bright"
			if lux <= 50 {
				status = "Dark"
			} else if lux <= 500 {
