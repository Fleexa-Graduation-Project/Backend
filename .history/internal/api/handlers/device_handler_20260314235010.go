...
func (h *DeviceHandler) GetDevices(c *gin.Context) {
    ...
    if states[i].Type == "light-sensor" {
        switch states[i].OperationalState {
        case "BRIGHT":
            states[i].Payload["light_status"] = "Bright"
        case "DARK":
            states[i].Payload["light_status"] = "Dark"
        case "NORMAL":
            states[i].Payload["light_status"] = "Normal"
        }
    }
}
...
func (handler *DeviceHandler) GetDeviceTelemetry(context *gin.Context) {
	...
	if isHotTier(period) {
		...
		if state.Type == "light-sensor" {
    stats, _ := telemetry.CalculateTempState(rawData, "light_level")
    response["min"] = stats.Min
    response["max"] = stats.Max
    response["average"] = stats.Average
}
		...
		if state.Type == "light-sensor" {
    recentData, _ := handler.TelemetryStore.GetTelemetryHistory(context.Request.Context(), deviceID, 0)
    stats, _ := telemetry.CalculateTempState(recentData, "light_level")
    response["min"] = stats.Min
    response["max"] = stats.Max
    response["average"] = stats.Average
}
