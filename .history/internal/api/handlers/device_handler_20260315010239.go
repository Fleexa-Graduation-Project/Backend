...
// addLightStatus derives a human-readable light_status from the operational_state
// already set during ingestion and injects it into the payload.
...
// addTempStats computes min/max/average from telemetry data and writes them into the response.
// Logs a warning if calculation fails (e.g. no data available).
...
func (handler *DeviceHandler) GetDeviceTelemetry(context *gin.Context) {
    ...
    if isHotTier(period) {
        // Pass the period cutoff to DynamoDB — reads only records within the window
        rawData, dbErr := handler.TelemetryStore.GetTelemetryHistory(context.Request.Context(), deviceID, 0, telemetry.PeriodCutoff(period))
        if dbErr != nil {
