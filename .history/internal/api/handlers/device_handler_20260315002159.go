...
func (h *DeviceHandler) GetDevices(context *gin.Context) {
	states, err := h.StateStore.GetAllStates(context.Request.Context())
	if err != nil {
...
func (handler *DeviceHandler) GetDeviceTelemetry(context *gin.Context) {
	...
	if isHotTier(period) {
		...
		if state.Type == "temp-sensor" {
			stats, statsErr := telemetry.CalculateTempState(rawData, metric)
			 if statsErr != nil {
                slog.Warn("failed to calculate temp stats", "device_id", deviceID, "metric", metric, "error", statsErr)
            }
			response["min"] = stats.Min
			response["max"] = stats.Max
			response["average"] = stats.Average
	 ...
	 if state.Type == "temp-sensor" {
            ...
            stats, statsErr := telemetry.CalculateTempState(recentData, metric)
            if statsErr != nil {
                slog.Warn("failed to calculate temp stats", "device_id", deviceID, "metric", metric, "error", statsErr)
            }
            response["min"] = stats.Min
