...
*Note: Device data endpoints are currently documented for the internal data plane. User authentication (Sign Up, Sign In, Password Reset) as shown in the UI mockups will be integrated via AWS Cognito or a dedicated Auth Service in a subsequent phase.*
...
*(All API endpoints in Sections 1-3 will eventually require an `Authorization: Bearer <token>` header.)*