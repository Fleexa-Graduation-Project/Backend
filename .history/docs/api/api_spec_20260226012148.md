...
Retrieves the live status of all provisioned smart home devices for the main dashboard.
...
Calculates aggregated data for a specific sensor over a requested time period to display on the UI stat blocks.
...
Retrieves critical events and warnings (e.g., Gas Leaks) to display in the "Warnings & Alerts" UI section.
...
Dispatches an instruction to an actuator (e.g., locking a door, changing A/C temperature). This writes to the `Fleexa_Commands` table, which the IoT system uses to message the physical device.
