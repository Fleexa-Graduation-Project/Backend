...
// Init DynamoDB 
	...
	if err != nil {
		panic(fmt.Errorf("failed to init device state: %w", err))
	}
...
func HandleRequest(ctx context.Context, event IoTEvent) error {
	...
	case "telemetry":
		...
		operationalState := devices.ExtractState(
			envelope.Type,
			envelope.Payload,
		)
		health := devices.EvaluateHealth(
			envelope.Type,
			operationalState,
		)
		...
		state := models.DeviceState{
			DeviceID:         envelope.DeviceID,
			Type:             envelope.Type,
			Status:           devices.ConnectionStatus(envelope.Timestamp),
			OperationalState: operationalState,
			Health:           health,
			LastSeenAt:       envelope.Timestamp,
		}
        
		return stateStore.UpdateState(ctx, state)
